...
class TestDatabaseConnection:
    ...
    @patch("mcp_server.db.connection.pyodbc.connect")
    ...
    @patch("mcp_server.db.connection.pyodbc.connect")
    def test_execute_query_as_dicts(self, mock_connect):
        """쿼리 결과를 딕셔너리 리스트로 반환하는지 테스트합니다."""
        mock_cursor = MagicMock()
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = [(1, "test1"), (2, "test2")]
        mock_cursor.__iter__ = lambda self: iter([(1, "test1"), (2, "test2")])
...
class TestGetConnection:
    ...
    def test_get_connection_returns_singleton(self):
